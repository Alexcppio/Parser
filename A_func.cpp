...
AnalyzeYandex::AnalyzeYandex(string sourceText)
{
	this->sourceText += sourceText;
}

void AnalyzeYandex::getTitle()
{
	string headerStart = "<span class = \"story__head - wrap\">";
	string headerEnd = "</h1";
	size_t foundStart = 0;
	size_t foundEnd = 0;
	if (sourceText.find(headerStart)) {
...
void AnalyzeYandex::getDate()
{
		
	//Москва, 12 сентября 2020, 04:20 — REGNUM Заявление - Образец

	string dateStart = "<div class = \"doc__text\">";

	size_t foundStart = 0;
	if (sourceText.find(dateStart)) { //c 9 по 25 символ
		foundStart = sourceText.find(dateStart) + dateStart.length();
	}
}

void AnalyzeYandex::getArticle()
{
	string articlePieceStart = "<div class = \"doc__text\">";
	string articlePieceEnd = "< / div>";
	//string hrefSkipStart = "";
	//string hrefSkipQuote = "";
	//string hrefSkipEnd = "";
	//string nobrSkipOpen = "";
	size_t foundStart = 0;
...
void AnalyzeYandex::save(string path)
{
	std::ofstream fin(path);
	fin << titleText << endl;
	fin << dateText << endl;
